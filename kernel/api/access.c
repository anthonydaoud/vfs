...
int addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        if (!(vma = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr)))) {
                return 0;
        }
...
{
        /* VM {{{ */
        uint32_t pn; /* Page number of the page we're checking permissions on */
        for (pn = ADDR_TO_PN(avaddr); pn <= ADDR_TO_PN(((uintptr_t) avaddr) + len - 1); pn++) {
                if (!addr_perm(p, (void *) PN_TO_ADDR(pn), perm)) {
                        dbg(DBG_VM, "range_perm: permissions insufficient for page starting at 0x%p\n",
                            PN_TO_ADDR(pn));
                        return 0;
                }
        }
        dbg(DBG_VM, ("range_perm: process has sufficient permissions\n"));
        return 1;

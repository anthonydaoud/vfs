...
kthread_clone(kthread_t *thr)
{
        ...
        if (NULL == (nt = slab_obj_alloc(kthread_allocator)))
                return NULL;
        char *new_kstack = alloc_stack();
        if (new_kstack == NULL) {
                return NULL;
        }
        nt->kt_kstack = new_kstack;

        KASSERT(!list_link_is_linked(&thr->kt_qlink));
        list_link_init(&nt->kt_plink);
        list_link_init(&nt->kt_qlink);
        nt->kt_wchan = NULL; /* It doesn't matter who was waiting on
                                the parent -> this is a new child
                                proc, so queue empty */

#ifdef __MTP__
        nt->kt_detached = thr->kt_detached;
        sched_queue_init(&nt->kt_joinq);
#endif
        nt->kt_retval = thr->kt_retval;
        nt->kt_errno = thr->kt_errno;
        nt->kt_cancelled = thr->kt_cancelled;
        KASSERT(KT_RUN == thr->kt_state);
        nt->kt_state = thr->kt_state;
        return nt;
        /* VM }}} */
        return NULL;
}

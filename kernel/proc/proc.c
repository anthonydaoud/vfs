...
proc_cleanup(int status)
{
        ...
        if ((!list_empty(&curproc->p_children)) && (PID_INIT != curproc->p_pid)) {
        ...
        } else if ((curproc->p_pid == PID_INIT) &&
...
switchout:
        ...
        /* Can't do this until now because there are filesystem operations
         * that might block happening inside proc_cleanup */
